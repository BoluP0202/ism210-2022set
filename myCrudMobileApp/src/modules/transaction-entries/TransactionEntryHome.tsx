...
dataSource.getRepository(TransactionEntry);
 ...
 useEffect(() => {
    ...
    <Text>Date={new Date(item.txnYear!, item.txnMonth!, item.txnDay!).toLocaleDateString()}</
   Text>
    ...
    ListHeaderComponent={
    () => (<View><Text h3 style={[styles.inputContainerStyle, { textDecorationLine:
   'underline' }]}>Entries so far</Text></View>)
    }
    ListFooterComponent={
    () => (<View style={{ backgroundColor: '#ccc', paddingBottom: 30, paddingTop: 3, alignContent:
   "flex-start" }}><Text style={{ fontSize: 15, fontStyle: "italic" }}>Copyright: Pius Onobhayedo</Text></View>)
    }
